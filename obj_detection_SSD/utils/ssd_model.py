...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        for (x, l, c) in zip(sources, self.loc, self.conf):
            ...
            # l(x)와 c(x)의 출력 크기는 [batch_num, 4*prior의 종류 수, featuremap 높이, featuremap 폭]
        ...
        if self.phase == "inference":  # 추론시
            ...
            #detection 클래수를 현 pytorch 버전에 맞게 수정함으로써 apply method 호출
            return self.detect.apply(output[0].to('cpu'), output[1].to('cpu'), output[2].to('cpu'))
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        # 손실함수 계산에 사용할 것을 저장하는 변수를 작성
        # conf_t_label: 각 BBox에 가장 가까운 정답 BBox(Ground Truth)의 라벨을 저장
        # loc_t: 각 BBox에 가장 가까운 정답 BBox(Ground Truth)의 위치 정보를 저장
        conf_t_label = torch.LongTensor(num_batch, num_dbox).to(self.device)
        loc_t = torch.Tensor(num_batch, num_dbox, 4).to(self.device)

        # loc_t와 conf_t_label에 BBox와 정답 어노테이션 targets(Ground Truth)를 match 시킨 결과를 덮어쓰기
        for idx in range(num_batch):  # 미니배치 루프

            # 현재 미니 배치의 정답 어노테이션의 BBox와 라벨을 취득
            
            # ground truth BBox의 localization 정보 (cx,cy,w,h), 각 이미지당 객체 갯수만큼 ground truth box갯수를 갖음 
            #ground truth[객체1의 localization 정보, 객체 2의 localization 정보, ...]
            truths = targets[idx][:, :-1].to(self.device)
            # 라벨 [물체1의 라벨, 물체2의 라벨, …]
            labels = targets[idx][:, -1].to(self.device)

            # 디폴트 박스(default boxes, priors, anchor boxes)를 새로운 변수로 준비
        ...
        # ----------
        # 물체를 감지한 BBox를 꺼내는 마스크를 작성, pos_maks:torch.Size([32,8732]), torch.bool
        pos_mask = conf_t_label > 0  # torch.Size([num_batch, 8732])

        # pos_mask를 loc_data 크기로 변형, pos_idx:torch.Size([32,8732,4]), torch.bool
        ...
        num_pos = pos_mask.long().sum(1, keepdim=True)  # 미니배치별 positive matching의 수
        ...
        # BBox의 손실 값이 큰 쪽에서 내림차순으로 정렬하여, BBox의 내림차순의 index를 loss_idx에 저장한다.
        ...
        # 지금 loss_idx[?]=0의 0은, 원래 loss_c의 요소 0번째라는 의미이다.
        # 즉 ?은 원래 loss_c의 요소 0번째는, 내림차순으로 정렬된 loss_idx의 몇 번째입니까?
        # 를 구하는 것이 되어, 결과적으로
        # ? = idx_rank[0] 은 loss_c의 요소 0번째가 내림차순으로 몇 번째인지 나타낸다.

        # 배경 BBox의 수 num_neg를 구한다. HardNegative Mining에 의해,
        # 물체 발견 BBox의 수 num_pos의 3배(self.negpos_ratio 배)로 한다
        # 만에 하나, BBox의 수를 초과한 경우에는 BBox의 수를 상한으로 한다
        ...
        pos_conf = conf_data[pos_idx_mask].view(-1, num_classes) #positive matching된 confidence score
        neg_conf = conf_data[neg_idx_mask].view(-1, num_classes) #negative matching된 confidence score
        # 마찬가지로 지도 데이터인 conf_t_label에서 pos와 neg만 꺼내어 conf_t_label_hnm으로
        # torch.Size([pos+neg]) 형태가 된다
        #conf_t_label_hnm = conf_t_label[(pos_mask+neg_mask).gt(0)]
        pos_conf_t_label = conf_t_label[pos_mask] #posive matching된 ground truth의 confidence score
        neg_conf_t_label = conf_t_label[neg_mask] #negative matching된 ground truth의 confidence score

        # confidence의 손실함수를 계산(요소의 합계=sum을 구한다)
        #loss_c = F.cross_entropy(conf_hnm, conf_t_label_hnm, reduction='sum')
        #loss_c = F.cross_entropy(conf_hnm, conf_t_label_hnm, reduction='sum')
        
        #positive matching된 BBox의 confidence score 값의 cross entropy 값
        loss_c_pos = F.cross_entropy(pos_conf, pos_conf_t_label, reduction='sum') 
        #Hard Negative mining BBox의 confidence score 값의 cross entropy 값
        loss_c_neg = F.cross_entropy(neg_conf, neg_conf_t_label, reduction='sum')
